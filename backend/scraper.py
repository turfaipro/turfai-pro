...
def get_html(url, timeout=25, retries=2, extra_headers=None):
    h = {}
    if extra_headers:
...
# ===================================================
# SOURCE 1 — API PMU OFFICIELLE (plusieurs endpoints)
# ===================================================

def source_api_pmu(date_str):
    # Essayer plusieurs variations de l'API PMU
    urls = [
        ...
        "https://turfinfo.api.pmu.fr/rest/client/61/programme/%s?meteo=true" % date_str,
    ]
    for url in urls:
        data = get_json(url)
        if not data:
...
def _extraire_pmu(reunion, course, date_str):
    ...
    tc = str(meteo.get("etatPiste", "BON")).upper()
    terrain_map = {
...
def source_geny(date_obj):
    url  = "https://www.geny.com/reunions-courses-pmu?date=%s" % date_obj.strftime("%Y-%m-%d")
    soup = get_html(url, extra_headers={"Referer": "https://www.geny.com/"})
    if not soup:
        return None
    try:
        for link in soup.find_all("a", href=True):
            txt  = link.get_text().upper()
            href = link.get("href","")
            if "QUINT" in txt or "quinte" in href.lower():
                course_url = href if href.startswith("http") else "https://www.geny.com" + href
                page = get_html(course_url, extra_headers={"Referer": url})
                if not page:
                    continue
                ...
                if len(partants) >= 5:
                    ...
                    log.info("Geny OK : %s — %d partants" % (titre, len(partants)))
                    return build_meta(
                        dt=datetime.combine(date_obj, datetime.min.time()),
                        nom=titre, ref="R1C3", lieu="Hippodrome",
                        heure="15h00", dist="2400m Plat Bon", alloc="50000 E",
                        terrain="Bon", discipline="Plat",
                        partants=partants, source="Geny",
                    )
    except Exception as e:
        log.warning("Geny : %s" % str(e))
    return None


# ===================================================
# SOURCE 4 — TURFOMANIA.FR
# ===================================================

def source_turfomania(date_obj):
    url  = "https://www.turfomania.fr/pronostics/%s/" % date_obj.strftime("%Y-%m-%d")
    soup = get_html(url, extra_headers={"Referer": "https://www.turfomania.fr/"})
    if not soup:
        return None
    try:
        for block in soup.find_all(["div","section","article"]):
            ...
            if "QUINT" in txt and len(block.find_all("tr")) >= 5:
                ...
                partants = _parse_table(block)
                if len(partants) >= 5:
                    log.info("Turfomania OK : %s — %d partants" % (titre, len(partants)))
                    return build_meta(
                        dt=datetime.combine(date_obj, datetime.min.time()),
                        nom=titre, ref="R1C3", lieu="Hippodrome",
                        heure="15h00", dist="2400m Plat Bon", alloc="50000 E",
                        terrain="Bon", discipline="Plat",
                        partants=partants, source="Turfomania",
                    )
    except Exception as e:
        log.warning("Turfomania : %s" % str(e))
    return None
            ...
            if endpoint == "arrivees":
                ...
                for rap in data.get("rapports", []):
                    if rap.get("typePari") == "E_SIMPLE_GAGNANT":
                        ...
                        break
            if len(arrivee) >= 5:
                log.info("Resultat PMU : %s" % str(arrivee[:7]))
                return {"arrivee": arrivee[:7], "source": "API PMU"}
        except Exception as e:
...
def scrape_quinte_du_jour():
    ...
    sources = [
        ...
        ("Equidia",    lambda: source_equidia(today)),
        ("Geny",       lambda: source_geny(today)),
        ("Turfomania", lambda: source_turfomania(today)),
    ]
